...
describe('normalizeClaudeRuntimeModelId', () => {
  ...
  test('prefers alias even when runtime versioned Sonnet ID appears in supported list', () => {
    const normalized = normalizeClaudeRuntimeModelId({
      runtimeModelId: 'claude-sonnet-4-5-20250929',
      supportedModelIds: new Set(['default', 'opus', 'haiku', 'claude-sonnet-4-5-20250929']),
    })
    expect(normalized).toBe('default')
  })
  ...
  test('maps human-readable sonnet runtime labels without supported model ids', () => {
    const normalized = normalizeClaudeRuntimeModelId({
      runtimeModelId: 'Sonnet 4.5',
      supportedModelIds: null,
      configuredModelId: 'sonnet',
    })
    expect(normalized).toBe('sonnet')
  })

  test('falls back to alias when supported model ids are unavailable', () => {
    const normalized = normalizeClaudeRuntimeModelId({
      runtimeModelId: 'Opus 4.6',
      supportedModelIds: null,
    })
    expect(normalized).toBe('opus')
  })
