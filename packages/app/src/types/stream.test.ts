...
describe('stream reducer canonical tool calls', () => {
  ...
  it('treats Nanoclaw TodoWrite like Claude TodoWrite', () => {
    const state = hydrateStreamState([
      {
        event: canonicalToolTimeline({
          provider: 'nanoclaw',
          callId: 'todo-write-nanoclaw',
          name: 'TodoWrite',
          status: 'running',
          input: {
        ...
        timestamp: new Date('2025-01-01T11:02:00Z'),
      },
    ])
    ...
    assert.strictEqual(tools.length, 0)
    assert.ok(todos)
    assert.strictEqual(todos.items[0]?.text, 'Task A')
  })

  it('preserves optimistic user message images when authoritative user message arrives', () => {
    const messageId = 'msg-user-images'
    const optimisticImages = [{ uri: 'file:///tmp/optimistic.jpg', mimeType: 'image/jpeg' }]
  ...
  it('reuses optimistic user message id when authoritative event omits messageId', () => {
    const optimisticId = 'msg-optimistic-no-id'
    const initialState: StreamItem[] = [
      {
        kind: 'user_message',
        id: optimisticId,
    ...
    const state = reduceStreamUpdate(initialState, event, new Date('2025-01-01T11:15:02Z'))
    ...
    assert.strictEqual(userMessages[0]?.id, optimisticId)
  })
})
