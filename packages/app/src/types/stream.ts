...
const OPTIMISTIC_USER_MESSAGE_REUSE_WINDOW_MS = 10_000

function isClaudeFamilyProvider(provider: AgentProvider): boolean {
  return provider === 'claude' || provider === 'nanoclaw'
}
...
function appendUserMessage(
  ...
  if (messageId) {
    ...
    const matchingOptimistic = [...state]
      .reverse()
      .find(
        (entry): entry is UserMessageItem =>
          entry.kind === 'user_message' &&
          entry.text.trim() === chunk.trim() &&
          Math.abs(timestamp.getTime() - entry.timestamp.getTime()) <=
            OPTIMISTIC_USER_MESSAGE_REUSE_WINDOW_MS
      )
    entryId = matchingOptimistic?.id ?? createUniqueTimelineId(state, 'user', chunkSeed, timestamp)
  }
      ...
      switch (item.type) {
        ...
        case 'tool_call': {
          const normalizedToolName = item.name
            ...
            .replace(/[.\s-]+/g, '_')
            .toLowerCase()
          if (isClaudeFamilyProvider(event.provider) && normalizedToolName === 'exitplanmode') {
            // ExitPlanMode is rendered via the plan permission prompt; avoid duplicating it in the timeline.
            break
          }

          if (
            isClaudeFamilyProvider(event.provider) &&
        ...
        case 'todo': {
          if (isClaudeFamilyProvider(event.provider)) {
